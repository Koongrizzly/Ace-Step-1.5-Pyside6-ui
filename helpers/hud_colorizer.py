...
# VRAM tokens like 'VRAM : 7.6/24.0 GB 32%'
_PAT_VRAM = re.compile(r"(VRAM\s*:?\s*.*?\s)(\d{1,3})%", re.IGNORECASE)

# VRAM amount pattern like 'DDR 5.3/24.0 GB'
_PAT_VRAM_AMOUNT = re.compile(
    r"(VRAM\s*:?\s*)(\d+(?:\.\d+)?)/(\d+(?:\.\d+)?)(\s*[GMK]i?B)",
    re.IGNORECASE,
)
...
def _colorize_percents(text: str) -> str:
    ...
    - DDR (system RAM) uses the generic _wrap_percent thresholds.
    - VRAM uses VRAM-specific 85/90/95 thresholds.
    """
    s = text
    ...
    s = _PAT_VRAM.sub(repl_vram, s, count=1)
