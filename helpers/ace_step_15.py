...
class ApiServerManager(QtCore.QObject):
    ...
    def _infer_framevision_root(self) -> Path:
            # Try to infer FrameVision root from env_python or project_root.
            try:
                p = Path(self.env_python)
            ...
            try:
                ...
                parts = [x.lower() for x in pr.parts]
                if "models" in parts:
                    idx = parts.index("models")
                    return Path(*pr.parts[:idx])
            except Exception:
                pass
            return Path.cwd()

    def _ensure_headless_bootstrap(self, api_server_py: Path) -> Path:
        """Ensure a *small* headless bootstrap server exists and return its path.

        This avoids generating a full copy of api_server.py. Instead we run a tiny
        bootstrap script that patches the gradio import chain *in memory*.
        """
        fv_root = self._infer_framevision_root()
        out_dir = fv_root / "presets" / "setsave"
        try:
            out_dir.mkdir(parents=True, exist_ok=True)
        except Exception:
            pass
        boot_py = out_dir / "acestep_api_server_headless_boot.py"

        boot_code = textwrap.dedent('''# Auto-generated by Ace Step 1.5 PySide UI (Keep in VRAM)
import argparse
import re
import sys
from pathlib import Path
...
def _patch_source(src: str) -> str:
    helper = \'\'\'
# --- Headless generation info (gradio-free) -----------------------------
...
def _build_generation_info_headless(
    ...
    if total <= 0 or n <= 0:
        return ""
    avg = total / max(1, n)
    return f"Total {total:.2f}s (avg {avg:.2f}s) | steps={inference_steps} | seed={seed_value}"
# ----------------------------------------------------------------------
\'\'\'

    src = re.sub(
        r'^\s*from\s+acestep\.ui\.gradio\.events\.results_handlers\s+import\s+_build_generation_info\s*$',
        helper + "\n# patched: use headless helper\n_build_generation_info = _build_generation_info_headless\n",
        src,
        flags=re.MULTILINE,
    )
...
def main() -> int:
    ...
    ap.add_argument("--repo", required=True, help="Path to ACE-Step-1.5 repo root (folder containing 'acestep').")
    args = ap.parse_args()

    repo = Path(args.repo).resolve()
    api_py = repo / "acestep" / "api_server.py"
    if not api_py.exists():
        print(f"[bootstrap] api_server.py not found: {api_py}", file=sys.stderr)
        return 2

    src = api_py.read_text(encoding="utf-8", errors="replace")
    patched = _patch_source(src)

    ns = {"__file__": str(api_py), "__name__": "__main__"}
    exec(compile(patched, str(api_py), "exec"), ns, ns)

    app = ns.get("app", None)
    if app is None:
        print("[bootstrap] 'app' not found after exec; nothing to run.", file=sys.stderr)
        return 3
...
''')
        try:
            cur = boot_py.read_text(encoding="utf-8", errors="replace") if boot_py.exists() else ""
            if cur != boot_code:
                boot_py.write_text(boot_code, encoding="utf-8", errors="replace")
        except Exception:
            pass
        return boot_py
    ...
    def start(self, lm_model: str = "") -> None:
        ...
        boot_py = self._ensure_headless_bootstrap(api_server_py)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self._current_theme = (getattr(self.settings, "ui_theme", "Evening") or "Evening").strip() or "Evening"
        self._build_ui()
    ...
    def _tick_generate_anim(self) -> None:
        ...
        try:
            ...
            if self._runner is None or not self.btn_stop.isEnabled():
        ...
        self._gen_anim_phase = (self._gen_anim_phase + 1) % 4
        n = self._gen_anim_phase
        # Keep suffix length constant (3 chars) so the button doesn't resize
        dots = ("." * n) + (" " * (3 - n))
        try:
            self.btn_run.setText(f"Generating{dots}")
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        try:
            ...
            target = fm.horizontalAdvance("Generating...") + 24
        ...
        if list_themes is not None and apply_theme is not None:
            ...
            for tname in list_themes():
                ...
                act.setChecked(tname == getattr(self, "_current_theme", "Evening"))
    ...
    def _apply_theme_simple(self, name: str) -> None:
        ...
        try:
            if apply_theme is None:
                return
            n = (name or "Evening").strip() or "Evening"
    ...
    def _on_theme_selected_simple(self, name: str, action_group: QtGui.QActionGroup) -> None:
        self._current_theme = (name or "Evening").strip() or "Evening"
        self._apply_theme_simple(self._current_theme)
        try:
    ...
    def _pull_ui_to_settings(self):
        ...
        # UI prefs
        try:
            s.ui_theme = (getattr(self, "_current_theme", "Evening") or "Evening").strip() or "Evening"
        except Exception:
            s.ui_theme = "Evening"
        try:
    ...
    def _set_busy(self, busy: bool):
        ...
        # Button label + tiny animation while generating
        try:
            if busy:
                self._gen_anim_phase = 0
                # Set an initial fixed-width label, then tick immediately
